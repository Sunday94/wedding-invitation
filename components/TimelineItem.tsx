...
const TimelineItem: React.FC<TimelineItemProps> = ({
  time, title, icon, location, address, description, details, imageSrc, isLast,
  accentColor, textPrimary, textSecondary, cardBg, cardBorder
}) => {
  return (
    <div className="relative pl-14 sm:pl-0 mb-10 last:mb-0 sm:min-h-[11rem]">
      {/* Vertical line connector */}
      {!isLast && (
        <div
          className="absolute left-[46px] sm:left-1/2 top-6 bottom-[-40px] w-[1px] sm:-translate-x-1/2"
      ...
      {/* Dot */}
      <div
        className="absolute left-[42px] sm:left-1/2 top-2.5 w-[9px] h-[9px] rounded-full z-10 shadow-sm sm:-translate-x-1/2"
        style={{ backgroundColor: accentColor }}
      ></div>

      {/* Time label */}
      <div className="absolute left-0 sm:left-auto sm:right-1/2 top-0.5 w-10 sm:w-20 text-right pr-2 sm:pr-0 sm:mr-5">
      ...
      <div className="sm:ml-[calc(50%+1.5rem)] sm:w-[calc(50%-1.5rem)]">
            ...
            <div className="space-y-4">
              ...
              {description && (
                <p
                  ...
                  style={{ color: textSecondary, borderColor: cardBorder }}
                >
                  {description}
                </p>
              )}
