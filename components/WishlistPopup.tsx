...
const formatGiftPrice = (price: number, currency: string | undefined): string => {
...
const WishlistPopup: React.FC<WishlistPopupProps> = ({ isOpen, onClose, accentColor, textPrimary, textSecondary, cardBg, cardBorder, items }) => {
    const [activeCategory, setActiveCategory] = useState("All");

    const categories = useMemo(() => {
        const dynamic = Array.from(new Set(items.map((item) => item.category || 'Others')));
        return ['All', ...dynamic];
    }, [items]);
                        ...
                        <div className="flex gap-2 overflow-x-auto no-scrollbar">
                            {categories.map((cat) => {
                                ...
                                return (
                                    ...
                                    >
                                        {CATEGORY_ICONS[cat] && <span className="material-icons text-xs">{CATEGORY_ICONS[cat]}</span>}
                                        {cat.toUpperCase()}
                                    </button>
                                );
                            })}
                ...
                <div className="flex-1 overflow-y-auto p-6 no-scrollbar">
                    <div className="grid grid-cols-1 gap-6">
                        {filteredItems.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="p-6 flex flex-col flex-1">
                                        ...
                                        <p className="text-2xl font-bold mb-6" style={{ color: textPrimary }}>
                                            {formatGiftPrice(item.price, item.currency)}
                                        </p>
                                        ...
                                        <div className="mt-auto pt-6 border-t flex items-center justify-center" style={{ borderColor: cardBorder }}>
                                            {item.sourceUrl ? (
                                                <a
                                                    ...
                                                    className="text-[11px] font-bold uppercase tracking-[0.2em] transition-colors hover:opacity-70"
                                                    style={{ color: textSecondary }}
                                                >
                                                    See more
                                                </a>
                            ...
                            <div className="rounded-[2rem] border border-dashed p-10 text-center" style={{ borderColor: cardBorder }}>
                                <span className="material-icons text-4xl opacity-30 mb-3" style={{ color: textSecondary }}>redeem</span>
                                <p className="text-sm font-medium" style={{ color: textSecondary }}>
                                    {items.length === 0 ? 'No gift requests yet' : 'No items in this category'}
                                </p>
                            </div>
                        )}
                    </div>
